...
MIT licensed. Art: the six governor portraits are a nano-banana (Gemini image) render of the
Softmax cog, one prop per seat, split from `scripts/art/source/governors_sheet.png` by
`scripts/art/split_governor_sheet.py`; the map plate and props are generated by

#!/usr/bin/env python3
"""Splits the nano-banana governor sheet into the six seat sprites.

scripts/art/source/governors_sheet.png is a single Gemini ("nano-banana") render
of the Softmax cog as six regional governors, one per seat colour, each
with its own large prop — red sash, blue gavel, green clipboard, yellow
megaphone, violet medkit, orange key — on a flat green backdrop.
This script keys the backdrop out with an edge flood fill (so green accents
on the green governor survive), splits the row into six, crops each to content,
pads to a square and writes 192 px RGBA sprites:

    python3 scripts/art/split_governor_sheet.py [outdir]

Default outdir is data/, the only art dir this repo uses (the viewer bundle
copies it in tools/build_replay_viewer.sh). Replaces the recoloured ctf
soldier portraits that scripts/art/recolor_sprite.py used to produce.
"""
...
SRC = os.path.join(os.path.dirname(__file__), "source", "governors_sheet.png")
ROLES = ["governor_%s_front.png" % c for c in
...
def main():
    ...
    print("governor sprites written to", outdir)
